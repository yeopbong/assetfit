...
The local application and its static explorer are ready for review. The public repository is [yeopbong/assetfit](https://github.com/yeopbong/assetfit). The complete release is pushed and its [static explorer](https://yeopbong.github.io/assetfit/) is publicly deployed. The publication checks below distinguish the initial release from the browser fallback repair. The independent release checkout contains the application, pinned dependencies, licensed inputs, measured candidates, real screenshots and reproducible evidence. Its source code uses the approved MIT license; sample licenses remain separate.
...
The successful environment was macOS arm64, Node 24.19.0, pnpm 11.19.0 and Chrome 152.0.7977.76 with SwiftShader. The declared minimum Node version is 22.13, matching the package manager's requirement. Node 22.13, full local processing on Linux or Windows, arbitrary GPUs and cross-machine identical rendering have not been verified. GitHub Actions passed frozen installation, typecheck, the default tests (48 passed and 2 opt-in renderer tests skipped), production build and static subpath verification on Ubuntu 24.04. This adds remote build/static evidence, not a claim that every local-processing platform is tested.
...
`.github/workflows/check.yml` runs install/test/build/static verification. `pages.yml` is manual: it builds and checks the static artifact before the deploy job. It follows [GitHub's custom Pages workflow](https://docs.github.com/en/pages/getting-started-with-github-pages/using-custom-workflows-with-github-pages) and the [project-site subpath model](https://docs.github.com/en/pages/getting-started-with-github-pages/what-is-github-pages). The public repository and MIT release are approved. The repository initialization was merged with the independent release history without rewriting it. A local build, a pushed repository, a successful deployment and an externally reachable site are separate states.
...
The initial published revision was `7b4b0dfde25cc1e0790add6127aa41524154ee41`: [release checks passed](https://github.com/yeopbong/assetfit/actions/runs/34014725338) and [Pages deployment passed](https://github.com/yeopbong/assetfit/actions/runs/34014741372). Public HTTPS browser checks verified the candidate table, three budgets, priority changes, rendered GLB inspection and an actual delivery with 6,901,595 asset bytes under an 8,500,000-byte budget. Every downloaded visual file was independently decoded and matched its recorded bytes, SHA-256 and recipe.

A publication check exposed incorrect fallback comparison when the browser cannot create WebGL: the original pane was missing and inspecting a candidate still showed the allocated candidate's recorded image. The repair uses the inspected candidate's recorded original/result pair, labels fixed-camera fallback clearly, and disables camera saving. It changes no source assets, candidates, measurements, allocation or export semantics. Historical candidate evidence remains valid. Earlier UI screenshots do not establish the fallback behavior. The local-acceptance source hashes remain a snapshot of that earlier revision; subsequent publication changes are preserved in Git history.

To verify the deployed site after installation, run `node scripts/verify-public.mjs https://yeopbong.github.io/assetfit/`. It checks actual drawn frames rather than merely counting canvases, along with a separate forced-WebGL-failure regression. Runtime evidence and actual downloads are saved under ignored `artifacts/public-verification/`.

The repaired revision `fc8448030e098a0f50b1558fb8ecfdd940cdc352` passed [release checks](https://github.com/yeopbong/assetfit/actions/runs/34015269536), [Pages deployment](https://github.com/yeopbong/assetfit/actions/runs/34015269578), and the complete public browser acceptance on September 6, 2026. [Publication evidence](publication.json) preserves exact remote job results, public file hashes, budget/priority selections, downloaded asset hashes and the separately verified no-WebGL fallback. The subsequent evidence commit only adds this record and clarifies that the sidebar's ZIP/metadata sizes refer to recorded local exports; the current browser download size is shown in its completion message. No historical encoding or search experiment was rerun.

At the 8,500,000-byte budget, model priority 10 selects 8,472,685 asset bytes and photo priority 10 selects 6,901,595. The verified photo-priority download was 9,723,095 ZIP bytes; visual assets were 6,901,595 bytes. This distinction is intentional: compressed reports, rendered comparisons and ZIP structure are additional. Initial loading requested two of 29 candidates and zero GLBs. Both normal interactive rendering and a separate unavailable-WebGL scenario passed, with no uncaught page errors.

![Public original and inspected candidate, with both real GLBs drawn](images/public-model-comparison.png)

![Public fallback displays the corresponding recorded original and inspected candidate](images/public-model-fallback.png)
